Promise.seriousRace = function(promise) {
	let indexPromise = promise.map((p, index) => p.catch(() => {throw index}))
	return Promise.race(indexPromise).catch(index => {
		let p = promise.splice(index, 1)[0]
		p.catch(e => console.log(e + " terjatuh,ahh sudahlah lanjutkan saja"))
		return Promise.seriousRace(promise)
	})
}
